...
- [ ] **R1.1 BoundedAudioChannel**: make admission release on dequeue (real
      ...
      to zero; no loss.
- [ ] **R1.2 Audio format metadata**: capture a genuinely mono buffer and
...
- [ ] **R1.3 Drain barrier**: establish final sequence + begin barrier before
...
- [ ] **R1.4 Control plane**: immediately-addressable session/control token
      before model preparation; release/cancel go direct to control plane;
      durable command buffering before orchestration starts.
- [ ] **R1.5 State machine**: legal `SessionControlModel` transitions are the
...
- [ ] **R1.6 Controller cleanup**: identity-checked `sessionDidFinish`
...
- [ ] **R2.1 Validate+insert as one transaction** against the same resolved
...
- [ ] **R2.2 AX timeout semantics**: never begin a write unless the remaining
...
- [ ] **R2.3 Remove automatic secure/fallback clipboard writes**: secure/
...
- [ ] **R3.1 Apple Speech**: one actor-owned finalization state; explicit
...
- [ ] **R3.2 WhisperKit**: chunked complete-audio finalization or visible hard
...
- [ ] **R4.1** Remove legacy `HistoryStore` writer; one actor repository;
...
- [ ] **R5.1** Globally unique placeholders or per-line span restore before
...
- [ ] **R6.1** Downloads default disabled until explicit onboarding consent;
...
- [ ] **R7.1** Deterministic integration tests instantiating the production
...
- [ ] **R8.1** Fix `ci_checks.sh` strict-concurrency (`|| true` swallow),
...
- [ ] **R8.2** Pin all third-party Actions to full SHAs; add missing qual
      scripts (focus_stress, secure_field_probe, accessibility_probe,
      hardware_profile, soak_1000) referenced by human-gates.md.
- [ ] **R8.3** Open a PR from the corrected branch; run full macOS/Xcode CI
...
- [ ] **R8.4** Re-run the root-owned final gate at the new terminal SHA and
