...
- [ ] **R8.3** Open a PR from the corrected branch; run full macOS/Xcode CI
      ...
      Round-6: NOT complete — requires a GitHub runner + push/PR; the local
      9-gate run is not exact-SHA CI evidence.
- [ ] **R8.4** Re-run the root-owned final gate at the new terminal SHA and
      ...
      at the exact validated commit). Round-6: NOT complete — external.
